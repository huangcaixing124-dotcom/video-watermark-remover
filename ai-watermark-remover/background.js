...
async function callBigmusicShareSave(messageId) {
  ...
  const endpoints = [
    {
      url: 'https://api-normal.doubao.com/alice/media/bigmusic/share_save?' +
    ...
    {
      url: 'https://www.doubao.com/creativity/share/share_save?' +
        'version_code=20800&language=zh&device_platform=web&aid=497858&real_aid=497858' +
        '&pkg_type=release_version&device_id=0&pc_version=3.14.6&region=CN&sys_region=CN' +
        '&samantha_web=1&use-olympus-account=1',
      body: { message_id: messageId }
    ...
    {
      url: 'https://www.doubao.com/creativity/share/save?' +
        'version_code=20800&language=zh&device_platform=web&aid=497858&real_aid=497858' +
        '&pkg_type=release_version&device_id=0&pc_version=3.14.6&region=CN&sys_region=CN' +
        '&samantha_web=1&use-olympus-account=1',
      body: { message_id: messageId }
  ...
  for (const ep of endpoints) {
    try {
      ...
      if (data.code === 0 && data.data) {
        ...
        console.log(`[AI去水印] bigmusicShareSave 成功: ${ep.url.substring(0, 60)}...`);
        return {
          success: true,
          share_id: shareId,
      ...
      console.warn(`[AI去水印] bigmusicShareSave 端点失败: ${ep.url.substring(0, 60)}... code=${data.code}`);
    } catch (e) {
      console.warn(`[AI去水印] bigmusicShareSave 端点错误: ${ep.url.substring(0, 60)}... ${e.message}`);
    }
  }
  return { success: false, error: '所有分享API端点均失败' };
}
